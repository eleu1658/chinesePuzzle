...
$('#puzzle-image').attr('src', 'src/images/Earth.png');
...
var puzzle = new ChinesePuzzle(config);
puzzle.zoom(-.3);
var path;
var movePath = false;
...
$('.help').mousedown(function() {
    if ($('.puzzel-canvas').css('display') == 'none') {
        $('.puzzel-canvas').show();
        $('.image-canvas').hide();
    }
    else {
...
$('.reload').click(function() {
    ...
    config.imgWidth = img.naturalWidth;
    config.imgHeight = img.naturalHeight;
    puzzle = new ChinesePuzzle(config);
    puzzle.zoom(-.3);
});
...
function ChinesePuzzle(config) {
    ...
    function getTileRaster(pos, sourceRaster, size, offset) {
